...
// $content['youtube'] will provide a full youtube url - embed that
		...
		<div class="row align-items-center gy-5 gx-0 gx-md-5">
			<div class="col-lg-6 <?php echo esc_attr( $content_order ); ?>">
				<div class="b-text-image__content">
					<?php if ( $title ) : ?>
						<h2 class="display-5 fw-bolder mb-4 ls-tight" data-aos="fade-in" data-aos-duration="500" data-aos-offset="-550">
							<?php echo esc_html( $title ); ?>
						</h2>
					<?php endif; ?>

					<?php if ( $text ) : ?>
						<div class="mb-4" data-aos="fade-in" data-aos-delay="250" data-aos-duration="500" data-aos-offset="-550">
							<?php echo wp_kses_post( $text ); ?>
						</div>
					<?php endif; ?>

					<?php if ( $button ) : ?>
						<div class="b-text-image__button" data-aos="flip-up" data-aos-delay="250" data-aos-offset="-550">
							<?php
			...
			<div class="col-lg-6 <?php echo esc_attr( $image_order ); ?>">
                ...
                <?php elseif ( $media_type === 'video' && $video ) : ?>
                    <div class="b-text-image__media-wrapper b-text-image__media-wrapper--video position-relative rounded-3 shadow overflow-hidden ratio <?php echo esc_attr( $ratio_class ); ?>">
                        <video class="object-fit-cover w-100 h-100" autoplay loop muted playsinline id="video-<?php echo esc_attr($block_id); ?>">
                ...
                <?php elseif ( $media_type === 'youtube' && $youtube ) : ?>
                    <div class="b-text-image__media-wrapper b-text-image__media-wrapper--youtube position-relative rounded-3 shadow overflow-hidden ratio <?php echo esc_attr( $ratio_class ); ?>">
                        ...
                        if ( preg_match('/src="([^"]+)"/', $iframe_html, $match) ) {
                            ...
                            // Add class/ID
                            $iframe_html = str_replace( '<iframe', '<iframe id="yt-' . esc_attr($block_id) . '"', $iframe_html );
                        }
                        echo $iframe_html; 
                        ?>
                    </div>
