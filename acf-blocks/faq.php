...
$questions = get_field('vragen');

if($use_categories) {
    $questions = get_field('vraagsoorten');
}
                    ...
                    <div class="col-lg-3 mb-4 mb-lg-0">
                        <div class="list-group faq-block__nav" role="tablist">
                            ...
                            foreach($questions as $question):
                                $question_tax = $question['vraagsoort'];
                                $term = get_term( $question_tax );
                                $cat_title = $term->name;
                    ...
                    <div class="col-lg-9">
                        <div class="tab-content" id="nav-tabContent-<?php echo esc_attr($block_id); ?>">
                            ...
                            foreach($questions as $question):
                                ...
                                $vragen = isset($question['vragen']) ? $question['vragen'] : [];
                            ?>
                                <div class="tab-pane fade <?php echo $is_active; ?>"
                                    ...
                                    <?php if (!empty($vragen)): ?>
                                        <div class="faq-block__list border rounded-3 overflow-hidden bg-white">
                                            <?php foreach ($vragen as $index => $vraag_id):
                                                $q = get_the_title($vraag_id);
                                                $a = get_field('antwoord', $vraag_id);
                                                $item_id = $block_id . '-' . $cat_slug . '-' . $index;
                                            ?>
                    ...
                    <div class="faq-block__content">
                         <?php if (!empty($questions) && is_array($questions)): ?>
                            <div class="faq-block__list border-topp">
                                <?php foreach ($questions as $index => $vraag_id):
                                    $q = get_the_title($vraag_id);
                                    $a = get_field('antwoord', $vraag_id);
